...
export const getServerSideProps: GetServerSideProps = async (context) => {
  const { categorySlug, seriesSlug, testSlug } = context.params!;
  try {
    const test = await api.get(`/mock-tests/${categorySlug}/${seriesSlug}/${testSlug}`);
    return { props: { test: test.test } }; // The API returns the test nested inside the join table object
  } catch (error) {
...
const TestPage: NextPage<TestPageProps> = ({ test }) => {
  ...
  const handleSubmit = async (isAutoSubmit = false) => {
    ...
    try {
      ...
      // Redirect to the user's dashboard to see their results
